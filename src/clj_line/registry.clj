...
(s/def ::altText (s/and ::not-empty-string #(>= 400 (count %))))
...
(s/def ::width number?)
(s/def ::height number?)
(s/def ::area (s/keys :req-un [::x ::y ::width ::height]))
(s/def ::grant_type #(= % "client_credentials"))
(s/def ::client_id ::not-empty-string)
(s/def ::client_secret ::not-empty-string)
(s/def ::access_token ::not-empty-string)
...
(s/def ::actions (s/coll-of ::imagemap-action :min-count 1))
...
(s/def ::issue-channel-access-token-request-body
  (s/keys :req-un [::grant_type ::client_id ::client_secret]))
(s/def ::revoke-channel-access-token-request-body
  (s/keys :req-un [::access_token]))
